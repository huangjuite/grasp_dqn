...
class DQNAgent:
        ...
        for frame_idx in range(1, num_frames + 1):
            ...
            if done:
                ...
                if score > np.min(score_track):
                    ...
                    self.save_model(score, frame_idx)
    ...
    def save_model(self, score, frame_idx, max_to_keep=10):
        model_name = './%s/s%04d_f%d.pth' % (
            self.writer.get_logdir(), int(score), frame_idx)
        torch.save(self.dqn.state_dict(), model_name)
        print("save model: %s" % model_name)

        files_dir = self.writer.get_logdir()
        files = os.listdir(files_dir)
        files = [f for f in files if "events" not in f]
